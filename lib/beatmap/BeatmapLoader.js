...
WOsu.BeatmapLoader.booleanKeys = [
    "Countdown", "LetterboxInBreaks", "StoryFireInFront", "WidescreenStoryboard",
    "DistanceSnap",
    ...
    while (index < lines.length) {
        ...
        if (line.trim().startsWith("[") && line.trim().endsWith("]")) {
            ...
            if (section == "HEADING") {
                ...
                var hitobj = WOsu.HitObject.getObject(map, line);
                mapObjects.push(hitobj);
                var lastTime = hitobj.time;
                for (var i = mapObjects.length - 2; i >= 0; i--) {
                    if (lastTime - mapObjects[i].endTime < mapData.StackLeniency * 1000.0) {
                        if (mapObjects[i].isCircle()) {
                            if (Math.abs(mapObjects[i].x - hitobj.x) <= 3.0 && Math.abs(mapObjects[i].y - hitobj.y) <= 3.0) {
                                lastTime = mapObjects[i].time;
                                mapObjects[i].stack--;
                            }
                        }
                        else if (mapObjects[i].isSlider()) {
                            if (Math.abs(mapObjects[i].x - hitobj.x) <= 3.0 && Math.abs(mapObjects[i].y - hitobj.y) <= 3.0) {
                                lastTime = mapObjects[i].time;
                                mapObjects[i].stack--;
                            }
                            else if (Math.abs(mapObjects[i].endX - hitobj.x) <= 3.0 && Math.abs(mapObjects[i].endY - hitobj.y) <= 3.0) {
                                lastTime = mapObjects[i].time;
                                mapObjects[i].stack++;
                            }
                        }
