...
WOsu.GameEvent.getEvents = function (obj, mechanics) {
    var events = [];
    var offset = mechanics.stack * obj.stack;
    if (obj.isCircle()) {

        // Only one event for circles
        var parent = new WOsu.GameEvent(WOsu.GameEvent.TYPE_CIRCLE, obj.time, obj.x + offset, obj.y + offset, obj);
        events.push(parent);
        ...
        // Slider start
        var parent = new WOsu.GameEvent(WOsu.GameEvent.TYPE_SLIDER_START, obj.time, obj.x + offset, obj.y + offset, obj);
        events.push(parent);

        // Slider end
        events.push(new WOsu.GameEvent(WOsu.GameEvent.TYPE_SLIDER_END, obj.endTime, obj.endX + offset, obj.endY + offset, obj, parent));
        ...
        for (var i = 0; i < obj.repeats; i++) {
            for (var j = 0; j < obj.ticks.length; j++) {
                ...
                events.push(new WOsu.GameEvent(WOsu.GameEvent.TYPE_SLIDER_TICK, offsetTime + t * obj.curveTime, obj.curveX[ti] + offset, obj.curveY[ti] + offset, obj, parent));
            }
        ...
        for (var i = 1; i < obj.repeats; i++) {
            ...
            if (i % 2 == 0) { // Repeat at the beginning
                events.push(new WOsu.GameEvent(WOsu.GameEvent.TYPE_SLIDER_REPEAT, offsetTime, obj.x + offset, obj.y + offset, obj, parent));
            }
            else { // Repeat at the end
                events.push(new WOsu.GameEvent(WOsu.GameEvent.TYPE_SLIDER_REPEAT, offsetTime, obj.endX + offset, obj.endY + offset, obj, parent));
            }
        }
        ...
        // Spinner start
        var parent = new WOsu.GameEvent(WOsu.GameEvent.TYPE_SPINNER_START, obj.time, obj.x + offset, obj.y + offset, obj);
        events.push(parent);
        // Spinner end
        events.push(new WOsu.GameEvent(WOsu.GameEvent.TYPE_SPINNER_END, obj.endTime, obj.x + offset, obj.y + offset, obj, parent));
