...
WOsu.ReplayLoader.loadRaw = function (bytes, finish, progress) {
    var replay = new WOsu.Replay();
    var index = 0;
...
WOsu.ReplayLoader.loadRawHex = function (data, finish, progress) {
    var bytes = new Uint8ClampedArray(~~(data.length / 2));
    for (var i = 0; i < bytes.length; i++) {
        bytes[i] = parseInt(data[2 * i] + data[2 * i + 1], 16);
    }

    return WOsu.ReplayLoader.loadRaw(bytes, finish, progress);
}
